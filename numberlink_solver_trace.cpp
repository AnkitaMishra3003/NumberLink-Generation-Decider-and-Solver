...
struct SolverOptions {
    bool trace = false;
    long long trace_limit = 50000;   // prevents accidental multi-GB logs
};

struct TraceStats {
    long long recursive_calls = 0;
    long long forced_moves = 0;
    long long branch_points = 0;
    long long trial_moves = 0;
    long long prune_failures = 0;
...
public:
    explicit NumberlinkSolver(vector<vector<int>> puzzle, SolverOptions options = {})
        : clue_raw(std::move(puzzle)), h((int)clue_raw.size()), w((int)clue_raw[0].size()), opt(options) {
        build_labels();
    ...
    bool solve() {
        stats = TraceStats();
        trace_lines = 0;
        trace_line(0, "START solver on " + to_string(h) + "x" + to_string(w) +
                         " grid with " + to_string(labels) + " pair(s)");
        bool ok = dfs(0);
        trace_line(0, string("END solver result = ") + (ok ? "SOLVED" : "IMPOSSIBLE"));
        if (opt.trace) {
            cerr << "[trace-summary] recursive_calls=" << stats.recursive_calls
                 << " forced_moves=" << stats.forced_moves
                 << " branch_points=" << stats.branch_points
                 << " trial_moves=" << stats.trial_moves
                 << " prune_failures=" << stats.prune_failures
...
private:
    ...
    string pos_str(Pos p) const {
        return "(" + to_string(p.r) + "," + to_string(p.c) + ")";
    }

    string label_str(int label) const {
        if (label >= 1 && label < (int)original_label.size()) {
            return "L" + to_string(original_label[label]);
        }
        return "L" + to_string(label);
    }

    string move_str(const Move& m) const {
        return label_str(m.label) + "/tip" + to_string(m.tip_idx) + " " +
               pos_str(m.from) + " -> " + pos_str(m.to) +
               (m.connect ? " CONNECT" : " EXTEND");
    }

    void trace_line(int depth, const string& msg) {
        if (!opt.trace) return;
        if (opt.trace_limit >= 0 && trace_lines >= opt.trace_limit) {
            if (trace_lines == opt.trace_limit) {
                cerr << "[trace] ... trace limit reached; remaining lines suppressed ...\n";
            }
            ++trace_lines;
            return;
        }
        ++trace_lines;
        cerr << "[trace] " << string(max(0, depth) * 2, ' ') << msg << "\n";
    }
    ...
    string first_prune_failure() const {
        if (!basic_degree_ok()) return "degree/local adjacency invariant failed";
        if (!stuck_tip_ok()) return "an unfinished tip has no legal move";
        if (!unfinished_pairs_reachable()) return "unfinished pair endpoints are disconnected";
        if (!empty_cell_future_degree_ok()) return "an empty cell has fewer than two future ports";
        if (!empty_components_ok()) return "empty component touches fewer than two active tips";
        return "none";
    }

    bool forced_propagate(vector<Move>& history, int depth) {
        while (true) {
            ...
            for (int id = 1; id <= labels; ++id) {
                ...
                for (int t = 0; t < 2; ++t) {
                    ...
                    if (mv.empty()) {
                        ++stats.prune_failures;
                        trace_line(depth, "DEAD forced check: " + label_str(id) +
                                          "/tip" + to_string(t) + " has 0 legal moves");
                        return false;
                    }
                    if ((int)mv.size() == 1) {
                        Move m = mv[0];
                        ++stats.forced_moves;
                        trace_line(depth, "FORCED " + move_str(m));
                        apply_move(m);
                        history.push_back(m);
                        if (!prune_ok()) {
                            ++stats.prune_failures;
                            trace_line(depth, "PRUNE after forced move: " + first_prune_failure());
                            return false;
                        }
                        progressed = true;
                        goto next_round;
    ...
    bool dfs(int depth) {
        ...
        trace_line(depth, "ENTER depth=" + to_string(depth));

        vector<Move> forced;
        if (!forced_propagate(forced, depth)) {
            trace_line(depth, "UNDO " + to_string(forced.size()) + " forced move(s) after dead end");
        ...
        bool all_done = true;
        for (int id = 1; id <= labels; ++id) {
            if (!done[id]) { all_done = false; break; }
        }
        if (all_done) {
            bool ok = final_state_ok();
            trace_line(depth, string("FINAL CHECK ") + (ok ? "passed" : "failed"));
            if (!ok) {
                ++stats.prune_failures;
                for (int i = (int)forced.size() - 1; i >= 0; --i) undo_move(forced[i]);
            }
        ...
        if (!choose_branch(label, tip_idx, moves)) {
            ++stats.prune_failures;
            trace_line(depth, "DEAD branch choice failed: no expandable active tip");
            for (int i = (int)forced.size() - 1; i >= 0; --i) undo_move(forced[i]);
            return false;
        }
        ...
        trace_line(depth, "BRANCH on " + label_str(label) + "/tip" + to_string(tip_idx) +
                          " at " + pos_str(tip[label][tip_idx]) +
                          " with " + to_string(moves.size()) + " candidate move(s)");

        int local_attempt = 0;
        for (Move m : moves) {
            ++local_attempt;
            ++stats.trial_moves;
            trace_line(depth, "TRY " + to_string(local_attempt) + "/" + to_string(moves.size()) +
                              " " + move_str(m));
            ...
            if (!prune_ok()) {
                ++stats.prune_failures;
                ++stats.backtracks;
                trace_line(depth, "PRUNE " + move_str(m) + " because " + first_prune_failure());
                undo_move(m);
                continue;
            }

            trace_line(depth, "PASS prune; descending after " + move_str(m));
            if (dfs(depth + 1)) {
                trace_line(depth, "ACCEPT " + move_str(m));
                // Keep final state, but do not undo forced moves on the success path.
                return true;
            }

            ++stats.backtracks;
            trace_line(depth, "BACKTRACK " + move_str(m));
            undo_move(m);
        }

        trace_line(depth, "EXHAUSTED branch; undo " + to_string(forced.size()) + " forced move(s)");
        for (int i = (int)forced.size() - 1; i >= 0; --i) undo_move(forced[i]);
        return false;
    }
};
    ...
    try {
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if (arg == "--trace" || arg == "-t") {
                ...
                if (i + 1 >= argc) throw runtime_error("--trace-limit requires a number.");
                options.trace = true;
                options.trace_limit = stoll(argv[++i]);
            } else if (arg == "--help" || arg == "-h") {
                cerr << "Usage: " << argv[0] << " [--trace] [--trace-limit N] < puzzle.txt\n";
                cerr << "Trace is written to stderr; solution remains on stdout.\n";
                return 0;
            } else {
        ...
        NumberlinkSolver solver(puzzle, options);
        if (!solver.solve()) {
            cout << "IMPOSSIBLE\n";
            return 0;
        }
