...
static void trace_log(const TraceOptions* trace, const string& msg) {
    if (trace && trace->enabled) cerr << msg << '\n';
}

static string orient_name(char orient) {
    if (orient == '-') return "horizontal";
    if (orient == '|') return "vertical";
    string s = "invalid(";
    s.push_back(orient);
    s.push_back(')');
    return s;
...
static Grid make_puzzle(int w, int h, int target_pairs, Mitm& mitm, RNG& rng,
    ...
    long long candidate_no = 0;

    while (true) {
        ++candidate_no;
        grid.clear();
        trace_log(trace, "[candidate " + to_string(candidate_no) + "] start: building a " +
                         to_string(w) + "x" + to_string(h) + " puzzle with K=" +
                         to_string(target_pairs));

        Path path = mitm.rand_path2(h, h, 0, -1);
        if (!grid.test_path(path, 0, 0)) {
            trace_log(trace, "[candidate " + to_string(candidate_no) +
                             "] reject: first boundary path overlaps or leaves grid");
            continue;
        }
        ...
        grid.set(0, 2*h, '/');
        trace_log(trace, "[candidate " + to_string(candidate_no) +
                         "] accept: first boundary path drawn, cells=" +
                         to_string(path.xys().size()));

        Path path2 = mitm.rand_path2(h, h, 0, -1);
        if (!grid.test_path(path2, 2*w, 2*h, 0, -1)) {
            trace_log(trace, "[candidate " + to_string(candidate_no) +
                             "] reject: second boundary path overlaps or leaves grid");
            continue;
        }
        ...
        grid.set(2*w, 2*h, '\\');
        trace_log(trace, "[candidate " + to_string(candidate_no) +
                         "] accept: second boundary path drawn, cells=" +
                         to_string(path2.xys(0, -1).size()));

        int pairs_initial = count_pairs(grid);
        trace_log(trace, "[candidate " + to_string(candidate_no) +
                         "] check: initial pairs=" + to_string(pairs_initial) +
                         ", target=" + to_string(target_pairs));
        if (test_ready_exact(grid, target_pairs)) {
            trace_log(trace, "[candidate " + to_string(candidate_no) +
                             "] SUCCESS: exact target reached before loop insertion");
            return grid.shrink();
        }
        ...
        for (int tries = 0; tries < LOOP_TRIES; ++tries) {
            ...
            if (orient != '-' && orient != '|') {
                trace_log(trace, "[candidate " + to_string(candidate_no) +
                                 "][loop attempt " + to_string(attempt_no) +
                                 "] skip: selected cell=(" + to_string(x/2) + "," +
                                 to_string(y/2) + ") has no valid tube orientation, got " +
                                 orient_name(orient));
                continue;
            }
            ...
            trace_log(trace, "[candidate " + to_string(candidate_no) +
                             "][loop attempt " + to_string(attempt_no) +
                             "] try: selected cell=(" + to_string(x/2) + "," +
                             to_string(y/2) + "), orientation=" + orient_name(orient));

            Path lp = mitm.rand_loop(orient == '-' ? 1 : -1);
            if (!grid.test_path(lp, x, y)) {
                trace_log(trace, "[candidate " + to_string(candidate_no) +
                                 "][loop attempt " + to_string(attempt_no) +
                                 "] reject: loop overlaps existing drawing or leaves grid, cells=" +
                                 to_string(lp.xys().size()));
                continue;
            }
            ...
            trace_log(trace, "[candidate " + to_string(candidate_no) +
                             "][loop attempt " + to_string(attempt_no) +
                             "] accept: loop inserted, pairs_now=" + to_string(pairs_now) +
                             "/" + to_string(target_pairs));
            ...
            if (pairs_now > target_pairs) {
                ...
                trace_log(trace, "[candidate " + to_string(candidate_no) +
                                 "][loop attempt " + to_string(attempt_no) +
                                 "] reject candidate: overshot target pair count");
                break;
            }
            if (test_ready_exact(grid, target_pairs)) {
                trace_log(trace, "[candidate " + to_string(candidate_no) +
                                 "][loop attempt " + to_string(attempt_no) +
                                 "] SUCCESS: exact target reached and validity filters passed");
                return grid.shrink();
            }
        }
        ...
        if (too_many_pairs) {
            trace_log(trace, "[candidate " + to_string(candidate_no) +
                             "] restart: too many pairs produced");
            continue;
        }
        trace_log(trace, "[candidate " + to_string(candidate_no) +
                         "] restart: loop-attempt budget exhausted without valid K-pair puzzle");
    }
}
...
int main(int argc, char** argv) {
    if (argc < 4) {
        cerr << "Usage: " << argv[0] << " WIDTH HEIGHT K [--count n] [--seed s] [--solve] [--trace]\n";
        ...
        cerr << "  --trace        : print every candidate and loop attempt to stderr\n";
        return 1;
    }
    ...
    trace_options.enabled = trace;
    if (trace) cerr << "[trace] seed=" << seed << ", count=" << count << "\n";

    for (int it = 0; it < count; ++it) {
        if (trace) cerr << "[trace] ===== puzzle " << (it + 1) << "/" << count << " =====\n";
        Grid puzzle = make_puzzle(w, h, target_pairs, mitm, rng, &trace_options);
